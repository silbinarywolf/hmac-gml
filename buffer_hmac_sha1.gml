...
/// @description Implementation of the HMAC algorithm using the SHA1 hash function. 
...
if string_byte_length(key_string) > hash_block_size {
    ...
    var hash = sha1_string_utf8(key_string);
...
var o_key_pad_buffer = buffer_create(hash_block_size + 2048, buffer_fixed, 1)
...
var hash = buffer_sha1(i_key_pad_buffer, 0, buffer_tell(i_key_pad_buffer));

// Convert from hex string to byte form as GML provides no way to get a raw byte buffer back.
var hash_size = string_byte_length(hash)
for (var i = 1; i <= hash_size; i += 2) {
	var byte_to_write = string_pos(string_char_at(hash, i+1), hex_lookup) + 
						(string_pos(string_char_at(hash, i), hex_lookup) * 16) - 17 
...
var result = buffer_sha1(o_key_pad_buffer, 0, buffer_tell(o_key_pad_buffer));
